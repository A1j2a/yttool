...
import { Mail, MessageCircle, Share2, Code2, Zap, CheckCircle } from 'lucide-react'
import { useToast } from '../hooks/useToast'
import Toast from '../components/Toast'

const SUPPORT_CARDS = [
  { icon: MessageCircle, title: 'Live Chat', desc: 'Chat with our team in real-time', action: 'Start Chat', color: '#00f5ff' },
  { icon: Mail, title: 'Email Support', desc: 'support@nexatools.io', action: 'Send Email', color: '#bf00ff' },
  { icon: Share2, title: 'Twitter / X', desc: '@NexaTools', action: 'Tweet Us', color: '#ff0080' },
]
    ...
    <PageWrapper>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          ...
          <p className="text-slate-500 max-w-xl mx-auto">Have a question or feedback? We'd love to hear from you.</p>
        </motion.div>
        ...
        <div className="grid sm:grid-cols-3 gap-5 mb-12">
          {SUPPORT_CARDS.map((card, i) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3" style={{ background: `${card.color}15`, border: `1px solid ${card.color}25` }}>
                <card.icon size={22} style={{ color: card.color }} />
              </div>
              <h3 className="text-white font-semibold mb-1">{card.title}</h3>
              <p className="text-slate-500 text-xs mb-3">{card.desc}</p>
              <button className="text-xs font-medium transition-colors" style={{ color: card.color }}>
                {card.action} →
              </button>
            </motion.div>
          ))}
        ...
        >
          {sent ? (
            <div className="text-center py-12">
              ...
              <button onClick={() => { setSent(false); setForm({ name: '', email: '', subject: '', message: '' }) }} className="mt-6 text-cyan-400 text-sm hover:underline">
